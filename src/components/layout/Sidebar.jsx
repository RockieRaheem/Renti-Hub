  ...
  return (
    <aside className={`bg-white border-r border-outline transition-all duration-200 hidden md:flex flex-col shrink-0 ${collapsed ? 'w-16' : 'w-56'}`}>
      ...
      <div className="px-2 py-3 border-t border-outline">
        {!collapsed ? (
          <div className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-surface-container transition-colors cursor-pointer">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium text-on-surface truncate leading-tight">James Kato</p>
              <p className="text-[11px] text-on-surface-muted truncate leading-tight">Portfolio Manager</p>
            </div>
          </div>
        ) : (
